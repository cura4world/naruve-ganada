...
최신: `naruve-ganada-dev-0.1.23.apk`
SHA256 `651f40418364ec3ff8c5fb5b7f1187067c9d6aca99b0a8e67e20c4d444b87244`
(이전 0.1.21 `90ed79d8413b422e626651d8f35f6463517408e03bdfcfc75c6118884408e498`)

**0.1.23은 네이티브가 바뀌었으므로 다시 설치해야 한다** — WebView 글꼴 배율 고정.
그 아래 웹 변경(마이크·로그 패널)은 0.1.21에서도 새로고침만으로 들어온다.
...
### "먹먹한 소리" — 고쳤다. 확인해 주세요 (0.1.23)

예시 음성이 녹음을 한 번 한 뒤부터 작고 먹먹하게 들렸다. 크롬과 APK 양쪽이
같았고 새로고침과 무관했다. **마이크 트랙이 열려 있는 동안 안드로이드가 오디오
경로를 통신 모드로 돌리는 것**으로 보고, 2026-08-18에 넣었던 "스트림 유지"를
되돌렸다. 이제 take가 끝나면 트랙을 놓고 다음 take에서 다시 연다.

**확인해 주실 것 (크롬과 APK 각각)**

| # | 하는 일 | 기대 |
|---|---|---|
| 1 | 녹음 → 예시 재생. **5회 반복** | 매번 깨끗하게 들린다 |
| 2 | 두 번째 녹음을 시작할 때 | **권한 다이얼로그가 뜨지 않는다** |
| 3 | 카카오톡 인앱에서 1·2 | 다이얼로그가 뜨면 알려 주세요 |

- 1이 여전히 먹먹하면 **가설이 틀린 것**이다. 그때는 AudioContext 샘플레이트와
  출력 장치 쪽을 본다 — 다음 차례로 넘긴다
- 2에서 다이얼로그가 뜨면 되돌림의 대가가 실제로 발생한 것이다.
  크롬·WebView는 같은 오리진의 권한을 기억할 것으로 봤지만 확인이 필요하다.
  **카카오 인앱이 특히 그렇다** — 트랙 유지를 넣은 이유가 그 브라우저였다

설정 탭의 **로그 보기**에 최근 20줄이 뜨고 **복사** 버튼이 클립보드로 넘긴다.
각 재생마다 `[example] play <파일> mic=<트랙상태> ctx=<AudioContext 상태>` 가 남는다.
이제 정상이라면 재생 시점의 `mic=`은 언제나 `none`이어야 한다.
`mic=live`가 보이면 어딘가 놓지 못한 길이 있는 것이고, 그 줄 옆에 경고가 함께 남는다.

### 글꼴 크기 — 시스템 설정을 무시한다 (0.1.23)

APK에서 크롬보다 글자가 커서 한 화면에 들어오지 않고 하단 탭 글자가 잘렸다.
WebView가 **시스템 접근성 글꼴 배율**을 곱해 적용하기 때문이다.
`MainActivity`에서 `setTextZoom(100)`으로 고정했다.

CSS의 `text-size-adjust:100%`도 함께 넣었지만 그것만으로는 부족하다 —
그 속성은 텍스트 자동확대(font boosting)를 끄는 것이고 `textZoom`과 다른 경로다.

**이것은 시스템 글꼴 설정을 무시하는 선택이다.** 첫 버전은 레이아웃 안정을
우선한다. 글자를 크게 쓰는 사용자에게는 불친절한 결정이므로,
**앱 안에 글꼴 크기 설정을 두는 것을 P6-B 이후 후보로 남긴다.**
그때 이 고정값을 그 설정과 연결한다.
