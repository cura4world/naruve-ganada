...
**타일 색은 `:root`의 `--tile-rgb`·`--tile-flip` 둘로만 정한다.**
먹물 검정 → `#1E3A8A`(0.1.20) → **`#172554`(0.1.22 확정)**. 만점 타일이
확실히 진해야 "다 맞았다"가 눈에 들어온다 — 100점 배경의 종이 대비가
8.94에서 12.61로 올랐고 0점 쪽은 1.23 → 1.26으로 사실상 그대로다. `app.js`의 `inkTile()`이 두 값을
CSS에서 읽으므로 색이 코드와 스타일 두 곳에 적히지 않는다 — 나중에 결과 카드
이미지를 그릴 때도 같은 값을 쓴다.

**색을 바꾸면 `--tile-flip`을 다시 재야 한다.** 같은 alpha라도 색마다 밝기가
달라 글자 반전 지점이 옮겨간다. 교차점은 검정 54.4 / `#1E3A8A` 65.8 / **`#172554` 57.5**다.
임계도 52 → 66 → **58**로 따라간다.
`node scripts/test_layout.js`가 WCAG 상대휘도로 교차점을 다시 재고 ±2를 넘으면 실패한다.

A/B 변형은 A로 확정했다(0.1.22). 낮은 어절에 seal 테두리를 덧대는 B안은
버렸다 — 채움만으로 이미 읽히고, seal은 "녹음 중"이라는 뜻과 겹쳤다.
`.low` 클래스만 남겨 뒀다. 나중에 낮은 어절을 다시 표시할 자리다.
...
- 치수는 `:root`의 `--mic-size` `--mic-sink` `--mic-rise` `--hint-h` 넷뿐이다.
  ...
  `--mic-sink`는 언제나 `--mic-size`의 절반이고, `.tab-gap`의 `min-width`가
  버튼과 종이 링(5px×2)을 담는다 — 좁으면 원이 옆 탭 위에 올라앉아 탭을 가로챈다.
  치수를 바꾸면 `node scripts/test_layout.js`가 이 셋을 다시 검사하고
...
## 억양 감점 (15.10 변경, 2026-08-19)

억양 방향을 틀리면 **총점에서 15를 깎는다.** 그전에는 억양이 자기 줄에만 있고
큰 숫자는 PronScore 하나였다 — 방향을 틀려도 숫자가 그대로면 억양은 점수가
아니라는 뜻이 된다.

깎는 조건은 좁다. **측정됐고(`measured`), 틀렸고(`ok === false`),
문장이 의문 또는 평서일 때만이다.** `score.js`의 `intonation()`은
`t === 'question'`이면 상승, **그 밖이면 하강**을 기대하는데 `exclam`이 그
"그 밖"에 싸잡혀 있다. 감탄문의 끝이 반드시 내려가야 하는 것은 아니므로
판정은 보여주되 점수는 깎지 않는다. 못 잰 경우(`ok === null`)도 그대로 둔다.

**깎인 값은 화면에만 있다.** 총점, 마지막 어절 타일(점수가 매겨진 마지막 것
하나), 그리고 결과의 "억양 −15" 줄뿐이다. `res.azure.words`는 손대지 않고
R2의 `.azure.json`과 dev 로그의 `pron`은 원값이다(16절). 원본을 고치면
캘리브레이션이 자기 꼬리를 물게 된다.

15는 잠정값이고 `score.js` 상단의 `INTONATION_PENALTY` 하나다.
`node scripts/test_intonation_penalty.js`가 조건과 원값 보존을 지킨다.

## 동의 2층과 저장 층

**동의 없이는 시작할 수 없다.** 온보딩의 필수 항목(채점을 위한 음성 처리와 익명
로그)을 체크해야 시작 버튼이 열린다. 선택 항목(모델 개선을 위한 5년 보관)은
기본 미체크이고, 없어도 모든 기능이 그대로 돈다 — 방침 2절이 그렇게 약속한다.

**동의는 R2 키의 맨 앞을 정한다.** `docs/js/api.js`가 `X-Naruve-Consent: base|extended`를
보내고 worker가 `base/{UUID}/…` 또는 `ext/{UUID}/…`에 쓴다. `base/`에는 R2
lifecycle 규칙 `base-7d`가 걸려 **7일 뒤 저장소가 스스로 지운다.** 층이 UUID보다
앞에 오는 이유는 R2 lifecycle 조건이 리터럴 접두어 하나뿐이기 때문이다 —
`{UUID}/base/`는 규칙으로 잡을 수 없다. **헤더가 없으면 base다.**

후속 제안은 **첫 85점 이상에서 한 번만** 뜬다. 못 나온 회차에 물으면 "이런 걸 왜
가져가나" 싶고 매번 물으면 성가시다. 거절하면 다시 묻지 않고 설정에서만 켤 수 있다.

선택 동의자에게는 **내 기록**(문장별 최고점)을 연다. `localStorage`의
`naruve.best`뿐이라 서버를 부르지 않는다 — 원가 0으로 만드는 체감 차이다(18절).

삭제 요청은 **서버 API가 없다.** 설정에서 UUID를 보여 주고 `mailto:`로 메일을 열어
준다. 방침 7절 문구가 그렇게 되어 있고, 받은 뒤 R2·KV에서 지우는 절차는
worker/README.md에 있다.

## 문장 이동과 마지막 자리

문장은 **컬렉션 안에서만** 움직인다. 화살표(`#prevSent`/`#nextSent`)와 수평
스와이프가 그것이고, **끝에서는 멈춘다** — 다음 컬렉션으로 넘기지 않는다.
컬렉션 이동은 Browse의 일이다.

"다음 문장" 버튼만 예외로 컬렉션 안을 순환한다. 계속 연습하겠다는 뜻이라
끝에서 막히면 흐름이 끊긴다.

**자동 재생은 버튼에만 있다.** 화살표와 "다음 문장"은 새 문장을 들려주고
**스와이프는 조용히 넘긴다** — 세로로 훑다가 손가락이 비스듬히 나가는 일이
있는데 거기에 소리까지 나면 놀란다.

마지막으로 보던 문장은 `naruve.pos`에 `{c, i, k}`로 남는다. 복원할 때
**문장 본문 `k`를 먼저 찾고** 인덱스 `i`는 폴백이다 — data.js가 바뀌면
인덱스가 밀리기 때문이다. 결과 화면은 복원하지 않는다. 점수는 다시 녹음해서 받는다.
...
설정 탭의 임시 바에 **로그 보기**가 있다. `[example]` 줄 최근 20줄을 띄우고
**복사** 버튼이 클립보드로 넘긴다 — 폰에서 긴 `<pre>`를 손으로 긁을 수 없다.
이 상자는 `.stage`(flex column)의 직계 자식이라 **`flex:none`이 필수다.**
빼면 stage 내용이 넘칠 때 한 줄 높이로 눌리고, 그렇게 눌리면
`overflow-y:auto`가 스크롤할 높이 자체를 못 가진다. 0.1.22에서 그렇게 안 보였다.
...
**소진 판정의 권위는 서버다. 캐시로 호출을 막지 않는다.** localStorage의
`naruve.credits`는 화면에 그릴 숫자일 뿐이고, 녹음이 끝나면 항상 `/score`를
부른 뒤 **402를 받고서야** 소진 상태로 간다. 0.1.18에서 반대로 했다가
테스터가 DEV_UUIDS에 올랐는데도 폰의 캐시 0 때문에 서버를 한 번도 부르지
못해 `credits:-1`을 받을 길이 없었다 — 새로고침해도 캐시라 풀리지 않았다.
소진일 때 이 호출은 Azure를 부르지 않는다. worker가 KV만 읽고 Azure·R2보다
앞에서 402를 돌려준다(실측 0.41초). `node scripts/test_credits.js`가 이 규칙을 지킨다.
...
**파일은 이미 들어 있다 (2026-08-19 배치 0 · 2026-08-26 배치 1).** docs/audio/m/ 과
docs/audio/f/ 에 mp3 400개(문장 200 × 목소리 2)가 있고 docs/audio/index.json이 그 목록이다.
타입캐스트 API(라이트 플랜, `ssfm-v30`)로 만들었고 목소리는 우성(남)·이현(여)다.
문장을 더할 때는 같은 규칙으로 파일을 넣고 index.json에 줄을 더한다 —
`scripts/tts/deploy_audio.py`가 둘을 함께 한다.

목록에 없거나 로드에 실패한 문장만 2번·3번으로 떨어진다. 폴백은 지우지 않는다 —
문장을 고치면 해시가 바뀌어 그 문장은 다시 파일이 없는 상태가 되기 때문이다.
그때는 콘솔에 warn이 찍힌다.

**`<audio>` 요소는 페이지에 하나뿐이다.** 재생마다 `new Audio()`를 만들면
폰에서 같은 파일이 겹쳐 울린다. 0.1.17이 그랬고, 원인은 "정지를 빠뜨렸다"가
아니라 **이전 재생의 늦은 `play()` 거부(AbortError)가 새 재생의 손잡이를 지운 것**이었다.
손잡이가 사라지면 이후 `stop()`이 무력해져 클릭마다 한 겹씩 쌓인다.
그래서 지금은 요소가 하나이고, 모든 콜백이 `live === mine`으로 자기가 아직
스피커의 주인인지 확인한 뒤에만 공유 상태를 건드린다. 늦게 도착한 콜백은
정상이며 **조용해야 한다.** 이 구조를 바꾸기 전에
`node scripts/tts/test_example_audio.js`를 돌린다.

**어느 목소리인지는 localStorage `naruve.voice`("m"/"f")가 정하고 기본은 "f"다.**
온보딩(P6-B)이 생기면 거기서 정한다. 지금은 설정 탭에 임시 토글이 있다.
플러그인은 그대로 둔다.
...
- ~~통합 비율 정리~~ — 닫혔다 (2026-08-19). DECISIONS 15.10.1이 가중 평균을
  폐기하고 페널티 방식을 확정했다. `SCORE_MIX`는 기록으로만 남는다.
...
- **문장 확장** — 500개 → 1000개 단계적으로. 200은 배치 1로 끝났다(2026-08-26, DECISIONS 20.5).
- ~~유료 TTS로 음성 생성~~ — 끝났다 (2026-08-19). 타입캐스트 라이트 플랜,
  문장 200 × 목소리 2 = 400개가 docs/audio/에 있다(배치 0 + 배치 1). **seed로 재현되지 않으므로
  그 파일들이 유일본이다** — 원본과 백업 두 곳(Naruve Data 폴더의 tts/ zip, R2 버킷의 tts/v1/)을 지우지 않는다.
...
**WebView는 시스템 글꼴 배율을 무시한다 (0.1.23).** `MainActivity`가
`webView.getSettings().setTextZoom(100)`으로 고정한다. 접근성 글꼴을 크게 잡은
기기에서 글자가 커져 한 화면에 안 들어오고 하단 탭 글자가 잘렸다.
**CSS의 `text-size-adjust`로는 막을 수 없다** — 그쪽은 텍스트 자동확대를 끄는
것이고 `textZoom`과 다른 경로다. 둘 다 넣었지만 실제로 듣는 것은 네이티브 쪽이다.
시스템 설정을 무시하는 선택이라 앱 안의 글꼴 크기 설정이 P6-B 이후 후보다.
...
## 작업 범위
지시받은 범위만 고친다. 발견은 보고하고, 다음 일은 묻지 않는다. 둘을 섞지 않는다.

**범위 밖에서 발견한 것은 고치지 말고 보고한다.** 파일:행을 함께 적는다.
근거는 그 자리에서 파일을 열어 확인한 것이어야 한다 — 전에 만든 표나 기억이
아니다. 이 보고는 이 프로젝트에서 가장 값어치 있는 산출물 중 하나다. 줄이지 않는다.
P52에서 "__pycache__ 17개 추적 중"이 사실이 아님을 CC가 스스로 확인해 보고했다.
실제로는 0건이었고, 앞선 조사가 find 결과를 추적 여부로 읽은 것이 원인이었다.

**다음에 무엇을 할지는 묻지 않는다.** 끝나면 결과만 보고하고 지시를 기다린다.
스스로 고르지도, 선택지를 만들어 고르게 하지도 않는다.
무엇을 할지는 Arga가 정하고 설계는 대화 Claude가 한다. CC는 실행이다.
P52 뒤 CC가 "다음 무엇을 할까요"를 선택지로 띄웠고, 그중 하나가 DECISIONS.md를
CC 판단으로 고쳐 PR까지 여는 것이었다. 승인 없이 고치지 않는 파일이다.

## 파일을 쓸 때
문자 그대로 남아야 하는 텍스트(Windows 경로·정규식·백슬래시 문자열)를 이스케이프가
풀리는 경로로 쓰지 않는다 — 확장되는 heredoc(`<<EOF`), `echo -e`, `printf` 형식 문자열.
파일 쓰기 도구로 직접 쓰거나 `<<'EOF'`로 감싼다.

쓴 뒤 제어문자를 확인한다. 탭과 CRLF는 빠져 있으므로 출력이 비어야 한다.
```
grep -nP '[\x00-\x08\x0B\x0C\x0E-\x1F]' <파일>
```

**눈으로 보는 diff는 이 손상을 잡지 못한다.** 확인 명령을 대신 쓰지 않는다.
README의 APK 경로가 BEL 둘로 깨진 채 몇 주를 지났다.

## 설계 원칙
- 학습앱이 아니라 점수앱. 첫 화면이 곧 기능이다. 로그인·온보딩·레벨테스트를 앞에 두지 않는다.
  (온보딩은 모국어·학습 단계 자가 신고 2문항까지 허용 — DECISIONS.md 16.4)
- 채점 기준은 **원어민다움(서울 표준 발음)** 이다 — DECISIONS 3.1 (2026-08-03 확정).
  "알아들을 수 있음"은 그때 기각됐다. 목표 분포는 원어민 92~100에 좁게, 학습자 30~90에
  넓게(3.2) — 아직 미측정이다.
...
## 출시 전 반드시 할 일
지금은 전부 개발값이 맞다. 출시 직전에 한 번에 바꾼다.

- **capacitor.config.json의 server 블록 제거.** 원격 URL만 띄우는 앱은 플레이
  심사에서 반려될 수 있다. 제거 후에는 docs/ 내용이 APK에 번들되며,
  그때부터 웹 수정마다 APK 재빌드가 필요하다.
- **capacitor.config.json의 `appName`을 정식 명칭으로.** 지금은
  "Naruve 가나다 (dev)"다. 스토어에 올릴 이름은 DECISIONS.md 19.4에 있다.
  **`appId`는 바꾸지 않는다** — 최초 AAB 업로드에서 영구 고정된다 (19.2).
...
- **지메일 "다른 주소에서 메일 보내기" 설정** (저장소 밖 · 계정 설정).
  support@naruve.app은 지금 수신 전용이라 문의에 답장할 수 없다. 이 주소가
  스토어에 표시되는 연락처이고 삭제 요청도 여기로 온다 (19.5 · 방침 7절).
...
`lb` 역할 라벨 14개, `tr` 흐림 어미 8개 — `tr`은 데이터만 있고 판정 코드에 연결되지 않았다
(DECISIONS 20.6 미결).

**마이크** 실제로 켜진다. 무음 절단·10초 상한·무음 take 무과금이 모두 동작한다.
**take가 끝나면 트랙을 놓는다 (0.1.23).** 놓는 곳은 `teardown()` 하나이고
다음 take는 `getUserMedia`를 다시 부른다.

~~스트림을 녹음 사이에 유지한다~~ ← 되돌렸다 (2026-08-19). 카카오톡 인앱이
문장을 바꿀 때마다 권한 다이얼로그를 띄워서 유지했었는데, **트랙이 live인 동안
예시 음성이 먹먹하고 작게 들렸다**(크롬·APK 양쪽). 마이크가 열려 있으면
안드로이드가 오디오 경로를 통신 모드로 돌리는 것으로 본다. 듣기가 망가지면
따라 할 표본이 망가진다 — 발음 앱에서는 그쪽이 더 큰 손해다.
**인앱 브라우저의 다이얼로그가 되살아나는지는 실기로 확인해야 한다.**
`audio.js`가 재생 직전에 열린 마이크를 한 번 더 닫고 그때 경고를 남긴다.
...
**개발자·테스터는 예외다** — Worker Secret `DEV_UUIDS`(콤마 구분 목록)에 오른
UUID는 KV를 읽지도 깎지도 않고 402도 맞지 않는다. 응답 `credits`가 `-1`이고
헤더에 `∞`로 뜬다. R2 저장과 로그는 그대로다. 등록 절차는 worker/README.md 2-1.
...
**예시 음성** 파일 재생이다. **음량을 정규화했다 (0.1.24)** — ffmpeg loudnorm
2-pass, 목표 I −16 LUFS / TP −1.5 dBTP. 우성이 이현보다 1.60 dB 낮던 것이
0.94 dB로 좁혀졌고 파일 간 편차는 7.88 → 4.30 dB다. 남은 편차는 원본의
true peak이 이미 −0.6 ~ −1.1 dBTP라 더 올리면 클리핑되는 고크레스트 클립들이다 —
리미터로 누르면 음절 강세가 뭉개져 표본이 망가지므로 누르지 않는다.
원본은 `..._final_20260819\`에, 정규화본은 `..._norm\`에 따로 둔다.
docs/audio/{m,f}/ 에 mp3 400개(33.3 MB), 배치 1 #77,
목록은 docs/audio/index.json. 타입캐스트 라이트 플랜으로 만들었고 목소리는
우성(남)·이현(여), 기본은 여자다. 파일이 없거나 로드 실패면 폰 내장 TTS로 떨어진다.
"다음 문장"을 누르면 그 문장을 자동으로 들려준다 — 첫 진입·컬렉션 전환·새로고침에는
울리지 않는다(브라우저 autoplay 정책과 같은 기준).
서비스워커는 이 파일들을 프리캐시하지 않는다. 첫 방문에 33.3 MB를 받게 하지 않으려는 것이고,
재생할 때 받아 `naruve-audio-v1` 캐시에 담는다(빌드가 올라도 지워지지 않는다).

**언어** en·ko 완전, id는 억양 피드백 6키만.

**결과 공유** `alert()` 자리표시자.

**온보딩과 설정 화면이 생겼다 (0.1.25).** 첫 실행에 온보딩 한 번 — 모국어·학습
단계(둘 다 건너뛸 수 있다, 16.4)·따라 할 목소리, 그리고 동의 2층. 설정 탭에는
목소리·표시 언어·채점 개선 참여·내 기록·내 식별자(복사 + 삭제 요청 메일)·방침
링크·버전이 있고, 개발 셸에서만 Reload·Log가 더 붙는다. 임시 `.voicebar`는 지웠다.
