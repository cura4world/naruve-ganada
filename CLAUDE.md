...
id는 지금 UI 열이 없고 음소 설명만 인도네시아어다 — 의도된 상태다.
인도네시아어 UI가 준비되면 ui.js에 `id: { ... }`를 더하기만 하면 된다.
...
**2층 발음 — 아직 자리표시자다.** Math.random이다.
`pronunciation.measured:false`이고 개발 모드에 "자리표시자"라고 찍힌다.
...
- **상황 태그 로케일 확장** — 지금은 ko/en 두 열이다. id를 채울 때 같이 채운다.
