...
**2층 발음 — 아직 자리표시자다.** `pronunciation()`이 SIMULATE 레벨 구간 안에서
Math.random을 돌린다. `pronunciation.measured:false`이고 개발 모드에
"자리표시자"라고 찍힌다.
**이 표시를 지우기 전에 실제 측정 엔진을 붙여라.**

**사용자에게는 통합 점수 하나만 보인다.** 점수앱이므로 큰 숫자가 간판이다.
"측정 준비 중" 같은 미완성 안내는 넣지 않는다.
**개발 모드에서는 두 점수를 분리해 보여준다.** 통합 점수만 보면 발음이
난수인 동안 F0 채점이 실제로 작동하는지 확인할 수 없기 때문이다.

비율은 `SCORE_MIX` 상수다. 지금 억양 30 : 발음 70.
억양은 마지막 음절의 단일 특징이고 발음은 전 음절에 걸치므로 정보량이
다르다. 다만 의문문을 내려 읽으면 실제로 못 알아들으므로 값을 물려야 한다 —
30%면 완전히 틀린 억양이 총점에서 약 15점을 가져간다.

억양 임계값(`INTO`)은 아직 아무도 실측하지 않은 잠정값이다.
개발 모드 표시를 탭하면 캘리브레이션 로그 전체가 클립보드로 복사된다.

**이 두 상수는 서로 물려 있다.** 발음 엔진이 붙는 날 `SCORE_MIX`를 다시 보는
것과 `INTO`를 실측으로 바꾸는 것은 같은 작업의 앞뒤다. 하나만 고치면
총점 눈금이 조용히 어긋난다.
...
- 예측 원자료와 함께 **실행 스크립트 원본과 설정 전체**를 같은 위치에 남긴다.
  하이퍼파라미터 표기만으로는 복원되지 않는다 (2026-08-17 실증: 3회 시도, 0.4178~0.4248 산포).
...
- **발음 측정 엔진** — 통합 점수의 70%가 아직 난수다. 나머지 보류 항목
  ...
  교체 지점은 score.js의 `pronunciation()` 하나다.
- **로마자 표기·뜻 가리기 토글** — 상급자용. 레벨이 올라가면 노출.
  로마자 표기의 현재 위치는 유지한다. 발음 앱이므로 발음 표기가
  눈에 잘 보이는 것이 맞다는 판단이다.
- **통합 비율 재검토** — 발음 채점 엔진이 실제로 붙는 날 `SCORE_MIX`를
  다시 본다. 지금의 30:70은 발음이 난수라는 전제 위에서 정한 값이다.
  실측 발음 점수가 들어오면 두 층의 신뢰도가 달라지므로 그대로 두면 안 된다.
