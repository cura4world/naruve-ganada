...
**타일 색은 `:root`의 `--tile-rgb`·`--tile-flip` 둘로만 정한다 (0.1.20).**
먹물 검정에서 진청 `#1E3A8A`으로 바꿨다. `app.js`의 `inkTile()`이 두 값을
CSS에서 읽으므로 색이 코드와 스타일 두 곳에 적히지 않는다 — 나중에 결과 카드
이미지를 그릴 때도 같은 값을 쓴다.

**색을 바꾸면 `--tile-flip`을 다시 재야 한다.** 같은 alpha라도 색마다 밝기가
달라 글자 반전 지점이 옮겨간다. 검정의 교차점은 54.4였고 진청은 65.8이라
임계를 52에서 66으로 올렸다. 52를 그대로 뒀다면 52점 타일에서 어두운 글자가
2배 가까이 잘 보이는데도(ink 5.49 vs paper 3.07) 흰 글자로 뒤집혔을 것이다.
`node scripts/test_layout.js`가 WCAG 상대휘도로 교차점을 다시 재고 ±2를 넘으면 실패한다.

`body.tile-a`/`body.tile-b`는 실기 비교용 임시 변형이다. B는 60점 미만 어절에
테두리만 seal을 덧칠한다(`!important` — 채움을 인라인으로 쓰기 때문이다).
설정 탭의 임시 바에 A/B 토글이 있고, P6-B에서 하나를 고르면 두 클래스와
토글을 함께 지운다.
